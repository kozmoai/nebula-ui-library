...
import { isNebulaKindValue } from '@/schemas/types/schemaValues'
...
export function useSchemaPropertyInput(schemaProperty: MaybeRefOrGetter<SchemaProperty>, propertyValue: Ref<SchemaValue>, propertyErrors: MaybeRefOrGetter<SchemaValueError[]>) {
  const input = computed(() => {
    ...
    if (isNebulaKindValue(propertyValue.value, 'json')) {
    ...
    if (isNebulaKindValue(propertyValue.value, 'jinja')) {
    ...
    if (isNebulaKindValue(propertyValue.value, 'workspace_variable')) {
    ...
    if (isNebulaKindValue(propertyValue.value, 'none')) {
      return withProps(SchemaFormPropertyInput, {
        property: property,
        value: propertyValue.value,
        errors,
