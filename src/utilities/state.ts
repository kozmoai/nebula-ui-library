...
import { nebulaStateNameTypes, nebulaStateNames } from '@/types/states'
import { capitalize } from '@/utilities/strings'

export function mapStateNameToStateType(stateName: string = 'Unknown'): { name: string, type: StateType | null } {
  const nebulaStateName = nebulaStateNames.find(name => stateName.toLowerCase() === name.toLowerCase())

  if (nebulaStateName) {
    const stateType = nebulaStateNameTypes[nebulaStateName]

    return {
      name: nebulaStateName,
      type: stateType,
    }
  }
