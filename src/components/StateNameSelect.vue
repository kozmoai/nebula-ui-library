...
<script lang="ts" setup>
  import { isArray, PSelect, SelectOption } from '@kozmoai/nebula-design'
  import { computed } from 'vue'
  import StateBadge from '@/components/StateBadge.vue'
  import { StateBadgeState } from '@/types/stateBadge'
  import { nebulaStateNames } from '@/types/states'
  ...
  const internalValue = computed({
    get() {
      ...
      const isAllExceptScheduled = props.selected.length === nebulaStateNames.length - 1
    ...
    set(value: string | string[] | null) {
      if (!value) {
        ...
        if (values.includes(allExceptScheduled)) {
          emits('update:selected', nebulaStateNames.filter((stateName) => stateName !== 'Scheduled'))
        } else {
  ...
  const options = computed<SelectOption[]>(() => {
    const stateNames = nebulaStateNames.map((stateName) => {
